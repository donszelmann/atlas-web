...
<xsl:key name="category" match="agenda_item" use="str:tokenize(category, '/')[2]"/>
...
<xsl:template match="/collection">
  <xsl:if test="*">
    ...
    <xsl:for-each select="./agenda_item[count(. | key('category', str:tokenize(category, '/')[2])[1]) = 1]">
      ...
      <ul style="margin:0 0 0.3em 0;padding:0;list-style:none;background-color:transparent;">
        <li style="margin:0;padding:0;line-height:1.2em;">
          <span style="font-weight:bold"><xsl:value-of select="str:tokenize(./category, '/')[2]" disable-output-escaping="yes"/></span>
        </li>

        <xsl:for-each select="key('category', str:tokenize(category, '/')[2])">
    ...
    <li style="margin:0;padding:0;line-height:1.2em;">
      <a href="http://indico.cern.ch/event/{./id}" style="text-decoration:none;color:#000000;outline:none;"
        ...
        <span style="margin-left:0.5em;font-weight:normal;">
          <xsl:value-of select="substring-after(substring-after(category, '/'), '/')" disable-output-escaping="yes"/>
        </span>
