 ...
 *   MAIN
 ***************/
$xmlString=getRawFeedXml($rawFeedUrl);      //get xml as a string
$dom = domxml_open_mem($xmlString);         //make a dom object from the string
$xsl=domxml_xslt_stylesheet_file($xslPath); //get an xsl transform file
$result=$xsl->process($dom);                //perform the transform
echo $xsl->result_dump_mem($result);        //return the result
